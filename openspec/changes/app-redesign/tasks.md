...
- [ ] T3.1 Modify `src/utils/schedule.ts` — add `groupEventsByDate()` utility: O(n) reduce by date, returns sorted `DayGroup[]` with computed dayNumber, totalDays, status (past/current/future)
