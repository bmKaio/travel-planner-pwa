...
function Places() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<PlaceCategory | 'all'>(initialCategory)
  // null = show the cities grid; a city id (or 'all') = show that city's places.
  // Deep-links carrying a category (e.g. from Coffee) skip the grid via 'all'.
  const [selectedCity, setSelectedCity] = useState<CityGroupId | 'all' | null>(
    initialCategory !== 'all' ? 'all' : null
  )
    ...
    return places.filter((place) => {
      ...
      const matchesCity =
        selectedCity === null || selectedCity === 'all'
          ? true
          : selectedCity === OTHER_CITY_ID
            ? nearestCityId(place) === null
            : nearestCityId(place) === selectedCity
  ...
  const renderMapPanel = (placesToShow: Place[], focusedId: string | null) => (
    <div className="relative h-[calc(100vh-120px)] w-full">
      {placesToShow.length === 0 ? (
        emptyState
      ) : (
        <Suspense fallback={<Loading label="Cargando mapa..." />}>
          <MapView places={placesToShow} focusedPlaceId={focusedId} />
        </Suspense>
      )}
      <a
  ...
  return (
    <div className="relative space-y-4">
        ...
        <div className="flex items-center gap-2">
          {!isCityGridView && (
            <button
              ...
              className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-slate-800"
              aria-label="Volver a ciudades"
            >
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">
            {isCityGridView ? 'Places' : selectedCityName}
          </h1>
          {!isCityGridView && (
      ...
      {isCityGridView ? (
        ...
        renderMapPanel(filteredPlaces, focusedPlaceId)
      ) : (
